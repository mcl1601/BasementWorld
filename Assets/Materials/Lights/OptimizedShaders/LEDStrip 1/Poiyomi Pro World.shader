Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro World/bf99e53072039bc459dc5947af799a70"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMAP 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(5,5,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(4.924577,4.924577,4.924577,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(4.924577,4.924577,4.924577,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(4.924577,4.924577,4.924577,1).rgb, float(0));
				#endif
				if (float(0))
				{
				...
				emissionColor0 = hueShift(emissionColor0, frac(float(0.021) + float(10) * _Time.x) * float(0));
				#if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
				...
				#endif
				applyALEmmissionStrength(poiMods, emissionStrength0, float4(0,1,0,0), float(2), float4(1,2,0,0), float(2), float(1));
				applyALCenterOutEmission(poiMods, poiLight.nDotV, emissionStrength0, float(0.132), float(2), float4(0,5,0,0), float(1));
