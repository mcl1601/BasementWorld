Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro World/c92f90d5afe80c54698501c69e7f0170"
{
	Properties
	{
 ...
 #define PROP_DISSOLVEDETAILNOISE 
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, in PoiLight poiLight)
			{
				...
				uv = decalUV(float(0), float4(1,1,0,0), float4(0.5,0.5,0,0), float(90) + decalRotation, float(0), decalScale, float4(0,0,0,0) +sideMod, float(0), poiMesh, poiCam);
				ddxuv = ddx(uv);
				ddyuv = ddy(uv);
				if (any(fwidth(uv) > .5))
				{
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(_DecalBlendAlpha + audioLinkDecalAlpha0));
				if (float(0))
				{
					alphaOverride += 1;
				...
				#endif
				poiFragData.emission += decalColor.rgb * decalColor.a * max(float(1) + audioLinkDecalEmission0, 0);
				#endif
				if (alphaOverride)
				{
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(2) == 1) // Basic
				{
					...
					edgeAlpha = remapClamped(da + float(0.209), da, noise) * (1 - dissolveAlpha);
				}
				else if (float(2) == 2) // Point to Point
				{
					...
					float3 currentPos;
					float distanceTo = 0;
					direction = normalize(float4(0,2.6,0,0) - float4(-1,0.5,0,0));
					currentPos = lerp(float4(-1,0.5,0,0), float4(0,2.6,0,0), dissolveAlpha);
					
					if (float(1) != 1)
					{
						float3 pos = float(1) == 0 ? poiMesh.localPos.rgb : poiMesh.vertexColor.rgb;
						distanceTo = dot(pos - currentPos, direction) - dissolveDetailNoise * dds;
						edgeAlpha = smoothstep(float(0.5) + .00001, 0, distanceTo);
				...
				if (float(0.209))
				{
					edgeColor = tex2D(_DissolveEdgeGradient, poiUV(float2(edgeAlpha, edgeAlpha), float4(3,3,0,0))) * float4(poiThemeColor(poiMods, float4(0,0.1416834,1,1).rgb, float(0)), float4(0,0.1416834,1,1).a);
					#ifndef POI_SHADOW
				...
				poiFragData.emission += lerp(0, dissolveToTexture * float(0), dissolveAlpha) + lerp(0, edgeColor.rgb * float(3.55), smoothstep(0, 1 - float(0) * .99999999999, edgeAlpha));
				#endif
			}
			#endif
			//ifex float(1)==0
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float4 mainTexture = UNITY_SAMPLE_TEX2D(_MainTex, poiUV(mainUV, float4(0.51,1.4,0,0)) + _Time.x * float4(0,0,0,0));
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.280335,0.280335,0.280335,1).rgb, float(0));
